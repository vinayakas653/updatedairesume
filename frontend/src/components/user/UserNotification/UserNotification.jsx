            ...
            <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
              {[
                ...
                >
                  {tab.label}
                  <span
                    className={`ml-1.5 text-xs ${filter === tab.key ? "text-slate-500" : "text-slate-400"}`}
                  >
                    ({tab.count})
                  </span>
                </button>
      ...
      const observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting && notification.isUnread) {
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                <h3
                  className={`text-sm ${notification.isUnread ? "font-semibold text-slate-800" : "font-medium text-slate-600"}`}
                >
                  {notification.title}
                </h3>
          ...
          <div className="flex items-end justify-between mt-auto">
            <div className="flex items-center gap-2 text-xs flex-wrap">
              <span
                className={`px-2 py-0.5 rounded-full ${typeConfig.bg} ${typeConfig.color} font-medium`}
              >
                {typeConfig.label}
              </span>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className={`px-6 py-5 ${typeConfig.bg} border-b border-slate-100`}>
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-xl bg-white/80 flex items-center justify-center shadow-sm`}
              >
